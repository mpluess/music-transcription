...
class PSOStringFretDetection(AbstractStringFretDetector):
    def __init__(self, tuning, n_frets):
        super().__init__(tuning, n_frets)
        self.fret_pos = (0, n_frets)

    def chord_distance(self, chord_a, chord_b):
        ...
        # fret distance
        min_fret_a, max_fret_a = self.fret_pos
        min_fret_b = 0
        ...
        string_distance = np.abs(np.median(active_strings_a) - np.median(active_strings_b)) / (len(chord_a) - 1)
    ...
    def predict_strings_and_frets(self, path_to_wav_file, onset_times_seconds, list_of_pitch_sets):
        ...
        for i in range(len(onset_times_seconds)-1):
            ...
            for chord_a in chords_a:
                ...
                for chord_b in chords_b:
                    d_matrix_row.append(1.0 - self.chord_distance(chord_a, chord_b))
        ...
        xopt, fopt = pso(f, lb, ub, args=(p_tab, p_d_tab), swarmsize=1000, maxiter=200, debug=False)
