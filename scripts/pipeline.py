...
beat_converter = SimpleBeatConverter(tempo_bpm=args.tempo, shortest_note=shortest_notes[args.shortest_note],
                                     beats_per_measure=float(args.beats_per_measure))
beats = beat_converter.transform(args.path_to_wav, onset_times_seconds, list_of_pitch_sets,
...
write_gp5(
    measures, tracks, beats, tempo=beat_converter.tempo, outfile=path_to_gp5, header=Header(
        track_title, '', '', '', '', '', '', '', '', ''
    )
)
